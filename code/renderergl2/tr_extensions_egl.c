...
extern qboolean EGL_ExtensionSupported(char *ext);
...
#define GLE(ret, name, ...) qgl##name = (name##proc *) eglGetProcAddress("gl" #name);
	...
	if (q_gl_version_at_least_3_0 || EGL_ExtensionSupported(extension))
	{
	...
	if (q_gl_version_at_least_3_2 || EGL_ExtensionSupported(extension))
	{
	...
	if( EGL_ExtensionSupported( extension ) )
	{
